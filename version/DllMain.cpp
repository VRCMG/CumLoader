...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
	...
	if (originaldll == NULL)
	{
		...
		{
			MessageBox(NULL, "Failed to Get System32 Directory!", "CumLoader", MB_ICONERROR | MB_OK);
			return FALSE;
		}
	}
	if (originaldll != NULL)
	{
		...
		{
			HINSTANCE cumloaderdll = LoadLibrary("CumLoader\\CumLoader.dll");
			if (cumloaderdll == NULL)
				MessageBox(NULL, "Failed to Load CumLoader.dll!", "CumLoader", MB_ICONERROR | MB_OK);
		}
		return TRUE;
	}
	else
		MessageBox(NULL, "Failed to Load version.dll!", "CumLoader", MB_ICONERROR | MB_OK);
	return FALSE;
}
