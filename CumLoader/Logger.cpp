...
void Logger::Log(const char* txt)
{
	...
	Console::Write("[");
	Console::Write("CumLoader", ConsoleColor_Magenta);
	Console::Write("] ");
...
void Logger::LogWarning(const char* txt)
{
	if ((MaxWarnings <= 0) || (WarningCount < MaxWarnings))
	{
		...
		if (CumLoader::DebugMode || !Console::HideWarnings)
		{
			Console::Write("[CumLoader] ", ConsoleColor_Yellow);
...
void Logger::LogError(const char* txt)
{
	if ((MaxErrors <= 0) || (ErrorCount < MaxErrors))
	{
		...
		Console::Write("[CumLoader] ", ConsoleColor_Red);
...
void Logger::LogCumError(const char* namesection, const char* txt)
{
	if ((MaxErrors <= 0) || (ErrorCount < MaxErrors))
	{
		LogTimestamp(ConsoleColor_Yellow);
...
void Logger::LogCumCompatibility(CumLoader_Base::CumCompatibility comp)
{
	LogTimestamp();
	Console::Write("[");
	Console::Write("CumLoader", ConsoleColor_Magenta);
	...
	switch (comp)
	{
	case CumLoader_Base::CumCompatibility::UNIVERSAL:
		LogFile << "Universal";
		Console::WriteLine("Universal", ConsoleColor_Cyan);
		break;
	case CumLoader_Base::CumCompatibility::COMPATIBLE:
		LogFile << "Compatible";
		Console::WriteLine("Compatible", ConsoleColor_Green);
		break;
	case CumLoader_Base::CumCompatibility::NOATTRIBUTE:
		LogFile << "No CumGameAttribute!";
		Console::WriteLine("No CumGameAttribute!", ConsoleColor_Yellow);
		break;
	default:
