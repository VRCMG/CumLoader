...
void Mono::CreateDomain()
{
	if (Domain == NULL)
	{
		...
		mono_runtime_set_main_args(CumLoader::CommandLineC, CumLoader::CommandLineV);
		Domain = mono_jit_init("CumLoader");
...
void Mono::FixDomainBaseDir()
{
	...
	if (!IsOldMono)
		mono_domain_set_config(Domain, CumLoader::GamePath, "CumLoader");
}
