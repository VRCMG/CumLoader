...
void AssertionManager::ThrowInternalError(std::string msg)
{
	if (!Result)
	{
		...
		if (CumLoader::DebugMode)
			MessageBox(NULL, msg.c_str(), "CumLoader - INTERNAL FAILURE", MB_OK | MB_ICONERROR);
		else
			MessageBox(NULL, "Please Post your Latest Log File\non #internal-failure in the CumLoader Discord!", "CumLoader - INTERNAL FAILURE!", MB_OK | MB_ICONERROR);
	}
}
