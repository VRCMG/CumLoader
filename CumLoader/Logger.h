...
public:
	...
	static void DebugLog(const char* txt) { if (CumLoader::DebugMode) Log(txt); }
	static void DebugLog(const char* txt, ConsoleColor color) { if (CumLoader::DebugMode) Log(txt, color); };
	...
	static void DebugLogWarning(const char* txt) { if (CumLoader::DebugMode) LogWarning(txt); }
	static void DebugLogWarning(const char* namesection, const char* txt) { if (CumLoader::DebugMode) LogWarning(namesection, txt); }
	static void DebugLogWarning(std::string txt) { DebugLogWarning(txt.c_str()); }

	static void DebugLogError(const char* txt) { if (CumLoader::DebugMode) LogError(txt); }
	static void DebugLogError(const char* namesection, const char* txt) { if (CumLoader::DebugMode) LogError(namesection, txt); }
	static void DebugLogError(std::string txt) { DebugLogError(txt.c_str()); }

	static void LogCumError(const char* namesection, const char* txt);
	static void LogCumCompatibility(CumLoader_Base::CumCompatibility comp);
