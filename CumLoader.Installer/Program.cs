...
using CumLoader.LightJson;
#pragma warning disable 0168

namespace CumLoader.Installer
{
    static class Program
    {
        internal static bool ShouldCheckForUpdates = false;
        internal static string Title = "CumLoader Installer";
        ...
        private static readonly string[] foldersToCleanUp = new string[] { "Logs", "CumLoader" };
        ...
        static void Main()
        {
            ...
            else
            {
                ...
                ManualZipPath = (Directory.GetCurrentDirectory() + "\\CumLoader.zip");
        ...
        static void Install_NoGUI()
        {
            ...
            {
                string version = null;
                JsonArray data = (JsonArray)JsonValue.Parse(webClient.DownloadString("https://api.github.com/repos/HerpDerpinstine/CumLoader/releases")).AsJsonArray;
                if (data.Count > 0)
                {
                ...
                {
                    MessageBox.Show("Installation failed; upload the created log to #cumloader-support on discord", Title);
            ...
            {
                MessageBox.Show("Failed to Get Version List; upload the created log to #cumloader-support on discord", Title);
                File.WriteAllText(Directory.GetCurrentDirectory() + $@"\MLInstaller_{DateTime.Now:yy-M-dd_HH-mm-ss.fff}.log", ex.ToString());
                Environment.Exit(-1);
            }
        }
        ...
        {
            SetDisplayText("Extracting CumLoader.zip...");
            Cleanup(dirpath);
        ...
        private static void Install_Silent(string dirpath, string selectedVersion)
        {
            ...
            webClient.DownloadFileAsync(new Uri("https://github.com/HerpDerpinstine/CumLoader/releases/download/" + selectedVersion + "/CumLoader.zip"), tempfilepath);
        ...
        internal static void Cleanup(string dirpath)
        {
            ...
            {
                DialogResult result = MessageBox.Show($"CumLoader could not remove old files.{Environment.NewLine}Please close the game then click retry to try again.{Environment.NewLine}If issue persists please click cancel to dump logs.", Program.Title, MessageBoxButtons.RetryCancel, MessageBoxIcon.Error);
        ...
        private static void CreateDirectories(string dirpath, string selectedVersion, bool legacy_install)
        {
            ...
            if (!Directory.Exists(Path.Combine(dirpath, "CumMods")))
                Directory.CreateDirectory(Path.Combine(dirpath, "CumMods"));
            if (!legacy_install && !selectedVersion.Equals("v0.2.2") && !Directory.Exists(Path.Combine(dirpath, "CumPlugins")))
                Directory.CreateDirectory(Path.Combine(dirpath, "CumPlugins"));
        }
        ...
        private static void Install_Legacy_02(string dirpath, string selectedVersion)
        {
            ...
            string tempfilepath = TempFileCache.CreateFile();
            webClient.DownloadFileAsync(new Uri("https://github.com/HerpDerpinstine/CumLoader/releases/download/" + selectedVersion + "/CumLoader" + (is_02 ? "_" : ".") + (File.Exists(Path.Combine(dirpath, "GameAssembly.dll")) ? "Il2Cpp" : "Mono") + ".zip"), tempfilepath);
            while (webClient.IsBusy) { }
            SetDisplayText("Extracting CumLoader...");
            Cleanup(dirpath);
            ExtractZip(dirpath, tempfilepath);
            ...
            if (is_02)
            {
                ...
                try { webClient.DownloadFileAsync(new Uri("https://github.com/HerpDerpinstine/CumLoader/raw/master/BaseLibs/UnityDependencies/" + mainForm.UnityVersion + ".zip"), tempfilepath4); while (webClient.IsBusy) { } } catch (Exception ex) { run_fallback = true; }
                if (run_fallback)
                {
                    ...
                    if (data.Count > 0)
                    {
                        ...
                        if (versionlist.Count > 0)
                        {
                            ...
                            webClient.DownloadFileAsync(new Uri("https://github.com/HerpDerpinstine/CumLoader/raw/master/BaseLibs/UnityDependencies/" + latest_version + ".zip"), tempfilepath4);
                            while (webClient.IsBusy) { }
                        }
                    }
                }
        ...
        private static void Install_Legacy_01(string dirpath)
        {
            SetDisplayText("Downloading CumLoader...");
            string tempfilepath = TempFileCache.CreateFile();
            webClient.DownloadFileAsync(new Uri("https://github.com/HerpDerpinstine/CumLoader/releases/download/v0.1.0/CumLoader.zip"), tempfilepath);
            while (webClient.IsBusy) { }

            SetDisplayText("Downloading Dependencies...");
            string tempfilepath2 = TempFileCache.CreateFile();
            webClient.DownloadFileAsync(new Uri("https://github.com/HerpDerpinstine/CumLoader/releases/download/v0.1.0/MonoDependencies.zip"), tempfilepath2);
            while (webClient.IsBusy) { }
        ...
        internal static void CheckForUpdates()
        {
            ...
            try
            {
                string response = webClient.DownloadString("https://github.com/HerpDerpinstine/CumLoader/raw/master/CumLoader.Installer/CumLoader.Installer.csproj");
                ...
                if (!response_p2.Equals(FileVersionInfo.GetVersionInfo(Process.GetCurrentProcess().MainModule.FileName).ProductVersion))
                {
                    ...
                    if (File.Exists(tempfilepath))
                        File.Delete(tempfilepath);
                    webClient.DownloadFileAsync(new Uri("https://github.com/HerpDerpinstine/CumLoader/releases/latest/download/CumLoader.Installer.exe"), tempfilepath);
                    while (webClient.IsBusy) { }
