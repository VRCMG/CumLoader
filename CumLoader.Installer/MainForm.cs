            ...
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (!string.IsNullOrEmpty(filePath))
                    {
                        ...
                        string existingFilePath = Path.Combine(Path.Combine(Path.GetDirectoryName(filePath), "CumLoader"), "CumLoader.ModHandler.dll");
        ...
        private void btnInstall_Click(object sender, EventArgs e)
        {
            ...
            new Thread(() =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show("Installation failed; upload the created log to #cumloader-support on discord", Program.Title);
        ...
        private void btnUninstall_Click_1(object sender, EventArgs e)
        {
            ...
            new Thread(() =>
            {
                ...
                catch (Exception ex)
                {
                    TempFileCache.ClearCache();
                    Program.SetDisplayText("ERROR!");
                    MessageBox.Show("Uninstall failed; upload the created log to #cumloader-support on discord", Program.Title);
                    File.WriteAllText(Directory.GetCurrentDirectory() + $@"\MLInstaller_{DateTime.Now:yy-M-dd_HH-mm-ss.fff}.log", ex.ToString());
                    Close();
                    Application.Exit();
                }
