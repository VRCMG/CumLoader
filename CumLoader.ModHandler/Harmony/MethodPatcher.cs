...
namespace Harmony
{
	public static class MethodPatcher
	{
		...
		{
			return CreatePatchedMethod(original, ("MELONLOADER_HARMONY_PATCH_" + CumLoader.BuildInfo.Version), prefixes, postfixes, transpilers);
		}
		...
		public static DynamicMethod CreatePatchedMethod(MethodBase original, string harmonyInstanceID, List<MethodInfo> prefixes, List<MethodInfo> postfixes, List<MethodInfo> transpilers)
		{
			try
			{
				...
				bool isIl2Cpp = CumLoader.UnhollowerSupport.IsGeneratedAssemblyType(original.DeclaringType);
